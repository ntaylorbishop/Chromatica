...
public:
	...
	//---------------------------------------------------------------------------------------------------------------------------
	TheRenderer(Vector2 aspect, float yFieldOfView, float nearClip, float farClip, const RGBA& color);
	...
	//VIEWPORT
	void SetViewport(const Vector2& loc, const Vector2& size);
	...
	void ClearScreen(const RGBA& col);
	void ClearScreen();
	...
	void UpdateVBO(const vector<Vertex2D_PCT>& verts, int numVerts, int vboID);
	...
	void SetCamera3D(const Matrix4& view);
	...
	int GetUniformLocation(int programID, const char* uniformName);
	void ShaderProgramSetUniform(Uniform* uniform);
	...
	//FRAME BUFFERS
	//---------------------------------------------------------------------------------------------------------------------------
	void FramebufferBind(const Framebuffer* fbo);
	void FramebufferCopyToBack(const Framebuffer* fbo);

	//---------------------------------------------------------------------------------------------------------------------------
	//FILTERING
	//---------------------------------------------------------------------------------------------------------------------------
	float GetMaxAntisotropicFilteringLevel();
...
private:
	...
	const Framebuffer* m_currentUserFBO;

	float m_maxAntisotropicFilteringLevel;
};