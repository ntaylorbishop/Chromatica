...
public:
	...
	uint32_t GetLocation() { return m_locInShader; }

	uint32_t m_locInShader;
protected:
	std::string m_name;
	eUniformType m_type;
};
...
public:
	Uniform_SAMPLER2D() : Uniform("SAMPLER2D", UNIFORM_SAMPLER2D) {	}
	Uniform_SAMPLER2D(const Sampler2D& data) : Uniform("SAMPLER2D", UNIFORM_SAMPLER2D) {
		m_data = data;
	}
	...
	void SetData(const Sampler2D& data) {
		m_data = data;
	}
